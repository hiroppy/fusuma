...
Fusuma uses [Prism.js](https://prismjs.com/) and [babel-plugin-prismjs](https://github.com/mAAdhaTTah/babel-plugin-prismjs). You can specify `plugins` and `theme` in `.fusumarc.yml`. The necessary syntax highlighting rule is automatically loaded based on the specified language in the [fenced code block](https://www.markdownguide.org/extended-syntax/#syntax-highlighting).
